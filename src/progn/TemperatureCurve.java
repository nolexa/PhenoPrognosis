...
public class TemperatureCurve {
  int startDecade;    // The decade where the curve started
  double[] points;    // Average temperatures
  int increment;

  public TemperatureCurve(int startDecade,int startFull,double[] dec) throws ArrayIndexOutOfBoundsException {
    int offset = startDecade - startFull;
    double[] decades = new double[dec.length - offset];
    System.arraycopy(dec,offset,decades,0,decades.length);
...
*/
  static double countMiddle(double[] source,int start,int stop) throws ArrayIndexOutOfBoundsException {
    double sum = 0;
